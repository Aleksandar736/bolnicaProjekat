...
<body>
    ...
    <div id="container2" class="row">
        ...
        <div id="container1" class="col-lg-10">
            <div class="card-header">
                <legend class="text-sm-center">Pacijenti</legend>
                <div class="form-group">
                    <div class="input-group input-group-merge">
                    <div class="pull-right"><input class="form-control" type="text" name="search" id="search" placeholder="Pretraga pacijenata"></div>                   
                    </div>
                </div>
            </div>    
            <div id="data-table">
            ...
            <table class="table table-striped table-sm table-responsive-sm">
                <thead>
                    <tr>
                        ...
                        <th>Adresa</th>
                        <th>Lekar</th>
                        <th>Datum dodele lekara</th>
                        <th>LBO</th>
                        <th colspan="2"><a href="/asistent/pacijent/forma_pacijent/" class="btn btn-success">DODAJ NOVOG PACIJENTA</a></th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                ...
                <tr>
                    ...
                    <td><?=$row->ime_pacijenta?> <?=$row->prezime_pacijenta?></td>
                    ...
                    <td><?=$row->lbo?></td>
                    <td><a href="/asistent/pacijent/forma_pacijent/<?=$row->pacijent_id?>" class="btn btn-primary">IZMENI</a></td>
                    <td>
                        <form method="POST" action="/asistent/pacijent/<?=$row->pacijent_id?>" >
                            <?php echo csrf_field(); ?>
    ...
    <script>
        ...
        function fetch_pacijent_data(query = '')
        {
        $('#data-table').load("{{ route('live_search_pacijent.action') }}?query=" + query + " #data-table")
        }

        $(document).on('keyup', '#search', function(){
        var query = $(this).val();
        fetch_pacijent_data(query);
        });
        });
	</script>     
</body>
